...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5005'}/api/auth/reset-password`, { 
        token, 
        newPassword 
      });
      setToast({ msg: data.message || "Password updated!", type: "success" });
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-6rem)] w-full max-w-lg items-center justify-center">
        <div className="glass-panel-strong w-full rounded-3xl border border-white/70 p-8 shadow-glass">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="mt-4 text-xs uppercase tracking-[0.4em] text-slate-500">Secure</p>
            <h2 className="mt-2 text-2xl font-semibold text-slate-900">Set New Password</h2>
